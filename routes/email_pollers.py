                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in ArgoDesk (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in ArgoDesk" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "argodesk_kind" if "argodesk_kind" in cols else "'scheduled' AS argodesk_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                argodesk_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-ArgoDesk-Origin"] = "argodesk-ui"
                outer["X-ArgoDesk-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", argodesk_kind or "scheduled")[:64]
                outer["X-ArgoDesk-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `argodesk-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `ARGODESK_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `argodesk-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("ARGODESK_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `ARGODESK_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (ARGODESK_INPROCESS_POLLERS=0); "
            "drive `argodesk-mail poll-scheduled` externally."
        )
        return
    import asyncio
