...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When ArgoDesk runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends ArgoDesk' own interpreter directory to PATH. If ArgoDesk itself is
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by ArgoDesk.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$ARGODESK_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $ARGODESK_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        runner_lines.append('  exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('  exit "$ARGODESK_PREFLIGHT_EXIT"')
...
def _append_serve_exit_code_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('ARGODESK_CMD_EXIT=$?')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $ARGODESK_CMD_EXIT ==="; exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('echo ""; echo "=== Process exited with code $ARGODESK_CMD_EXIT ==="')
        runner_lines.append('exit "$ARGODESK_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[argodesk] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('      _argodesk_has_cudart() {')
    ...
    runner_lines.append('        echo "[argodesk]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    else')
    runner_lines.append('      echo "[argodesk] WARNING: no HIP/CUDA toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[argodesk]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[argodesk]   Install ROCm for AMD GPUs or vLLM/CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
...
def _llama_cpp_rebuild_cmd() -> str:
    ...
    return (
        ...
        'echo "[argodesk] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\argodesk-sessions"
