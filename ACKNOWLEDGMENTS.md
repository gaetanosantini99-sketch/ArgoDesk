...
ArgoDesk stands on the shoulders of a lot of open-source work. This file
credits the projects whose code, assets, or designs are included in or
adapted by this product, and notes their licenses.

If you believe something here is mis-attributed or missing, please contact the
ArgoDesk team — it will be corrected promptly.
...
- **[Tongyi DeepResearch](https://github.com/Alibaba-NLP/DeepResearch)** by
  **Alibaba-NLP / Tongyi Lab** — the multi-step deep-research agent pipeline.
  Copyright © Alibaba-NLP / Tongyi Lab. **Apache-2.0.** Adapted for ArgoDesk's
...
and run alongside ArgoDesk on `docker compose up`. They are not modified —
just composed.
...
ArgoDesk talks to these over the network/API. They are **not** distributed
with this product; their licenses do not bind this codebase, but they deserve
credit:
...
- **`caldav`** (Python lib) is **dual-licensed GPL-3.0-or-later OR Apache-2.0**.
  ArgoDesk uses it under **Apache-2.0**, which is permissive and MIT-compatible.
...
Most of ArgoDesk's code was written *with* AI models, not just by a human.
