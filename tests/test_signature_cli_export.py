...
def _load_signature_cli(monkeypatch):
    ...
    path = Path(__file__).resolve().parent.parent / "scripts" / "argodesk-signature"
    loader = importlib.machinery.SourceFileLoader("argodesk_signature_cli_under_test", str(path))
