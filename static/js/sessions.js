...
let _sortMode = Storage.get('argodesk-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'ArgoDesk Chat';
...
const FOLDER_STATE_KEY = 'argodesk-folder-state';
const FOLDER_ORDER_KEY = 'argodesk-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to ArgoDesk kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'ArgoDesk Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('argodesk-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('argodesk-session-sort', mode);
  else Storage.remove('argodesk-session-sort');
  renderSessionList();
}
