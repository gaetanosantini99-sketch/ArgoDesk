...
const _rememberedDockKey = (id) => `argodesk-modal-remembered-dock-${id}`;
...
function _emitModalOpened(id, modal) {
  try {
    window.dispatchEvent(new CustomEvent('argodesk:modal-opened', {
      detail: { id, modal },
    }));
  } catch (_) {}
}
...
const _DOCK_STORAGE_KEY = 'argodesk.mobileDockState.v1';
...
function _renderDock() {
  ...
  for (const id of renderIds) {
    ...
    chip.innerHTML = `
      ...
      <span class="minimized-dock-x" title="Chiudi">×</span>
    `;
