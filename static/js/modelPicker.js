...
const RECENT_KEY = 'argodesk-model-recent';
const FAVORITES_KEY = 'argodesk-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  const _collapsedProviders = new Set(_loadList('argodesk-model-collapsed'));
  ...
  function _populate(filter) {
    ...
    function _addRow(m) {
      ...
      if (m.stale) {
        ...
        row.title = `Local server appears offline: ${m.staleReason}. Click to try anyway, or relaunch in Modelli locali.`;
      }
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('argodesk-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('argodesk:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('argodesk:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `argodesk-model-favorites[0]`
