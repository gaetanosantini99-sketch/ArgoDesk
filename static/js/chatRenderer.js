...
const _ACTION_RECENTS_KEY = 'argodesk-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'argodesk-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'ArgoDesk';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'ArgoDesk' : shortModel(resolvedModel);
