...
async function _fetchEvents(start, end, force) {
  ...
  const fetchPromise = fetch(`${API_BASE}/api/calendar/events?start=${start}&end=${end}`, { credentials: 'same-origin' })
    ...
    .catch(e => { console.error('Calendario: recupero eventi non riuscito', e); });
...
async function _fetchCalendars() {
  ...
  } catch (e) { _calendars = []; _calendarsError = e.message || 'Connessione non riuscita'; }
...
async function _syncCaldav(interactive) {
  try {
    ...
    if (interactive) return { errors: [e.message || 'Sincronizzazione non riuscita'] };
  }
}
...
async function _createEvent(data) {
  ...
  fetch(`${API_BASE}/api/calendar/events`, {
    ...
    delete _allEvents[tempUid];
    if (_open) _render();
    if (window.uiModule) window.uiModule.showError('Creazione dell’evento non riuscita: ' + (e?.message || 'sconosciuto'));
  });
  return { uid: tempUid };
}
...
async function _updateEvent(uid, data) {
  ...
  fetch(`${API_BASE}/api/calendar/events/${encodeURIComponent(uid)}`, {
    ...
    if (_open) _render();
    if (window.uiModule) window.uiModule.showError('Aggiornamento dell’evento non riuscito: ' + (e?.message || 'sconosciuto'));
  });
  return { ok: true };
}
...
async function _deleteEvent(uid) {
  ...
  fetch(`${API_BASE}/api/calendar/events/${encodeURIComponent(uid)}`, {
    ...
    if (window.uiModule) window.uiModule.showError('Eliminazione dell’evento non riuscita: ' + (e?.message || 'sconosciuto'));
    if (_open) _render();
  });
  return { ok: true };
}
...
function _showEventMoreMenu(ev, anchor) {
  ...
  dropdown.appendChild(_item(_editIcon, 'Modifica', () => {
    closeMenu();
    _showEventForm(ev);
  }));

  dropdown.appendChild(_item(_trashIcon, 'Elimina', async () => {
    closeMenu();
    const name = ev.summary ? `"${ev.summary}"` : 'this event';
    const ok = await uiModule.styledConfirm(`Eliminare ${name}?`, { confirmText: 'Elimina', danger: true });
    if (!ok) return;
...
async function _createEventReminder(ev, dueDate) {
  ...
  const startFmt = ev.all_day
    ? new Date(ev.dtstart).toLocaleDateString('it-IT', { weekday:'short', month:'short', day:'numeric' })
    : new Date(ev.dtstart).toLocaleString('it-IT', { weekday:'short', month:'short', day:'numeric', hour:'numeric', minute:'2-digit' });
  const summary = ev.summary || '(senza titolo)';
  const loc = ev.location ? ` @ ${ev.location}` : '';
  const text = `${summary}${loc} — ${startFmt}`;
  const payload = {
    title: `Promemoria: ${summary}`,
    note_type: 'todo',
  ...
  try {
    ...
    const fmt = dueDate.toLocaleString('it-IT', { month:'short', day:'numeric', hour:'numeric', minute:'2-digit' });
    if (uiModule.showToast) uiModule.showToast(`Promemoria impostato per ${fmt}`);
    try { window.notesModule?.refreshDueBadge?.({ force: true }); } catch {}
    if ('Notification' in window && Notification.permission === 'default') {
      try { Notification.requestPermission(); } catch {}
    }
  } catch (e) {
    if (uiModule.showError) uiModule.showError('Creazione del promemoria non riuscita');
  }
}
...
const BADGE_SEEN_KEY = 'argodesk-calendar-badge-seen';
...
function _renderEmpty() {
  ...
  body.innerHTML = `
    <div class="cal-empty-state">
      ...
      <div class="cal-empty-title">${hasError ? 'Calendario non disponibile' : 'Nessun calendario ancora'}</div>
      <div class="cal-empty-msg">${hasError ? _e(_calendarsError) : 'Crea un calendario locale, importa un file .ics o sincronizza tramite CalDAV.'}</div>
      ${hasError ? `
        <button class="cal-btn cal-btn-primary" id="cal-goto-settings">Apri Impostazioni</button>
      ` : `
        <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-top:4px;">
          <button class="cal-btn cal-btn-primary" id="cal-empty-new">Nuovo calendario</button>
          <button class="cal-btn" id="cal-empty-import">Importa .ics</button>
        </div>
        <div style="margin-top:10px;font-size:11px;opacity:0.55;">Oppure <a href="#" id="cal-empty-caldav" style="color:var(--accent, var(--red));text-decoration:none;font-weight:600;">configura la sincronizzazione CalDAV</a>.</div>
      `}
    </div>`;
  ...
  // has the "Nuovo calendario" button and the .ics file picker. Import
...
function _headerHTML() {
  ...
  <div class="cal-quickadd-row" id="cal-quickadd-row">
    <input
      ...
      autocomplete="off"
    />
    <span class="cal-quickadd-hint" id="cal-quickadd-hint" aria-hidden="true"><span class="qa-hint-accent">Aggiunta rapida</span> <svg class="qa-hint-enter" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="9 10 4 15 9 20"/><path d="M20 4v7a4 4 0 0 1-4 4H4"/></svg></span>
...
function _filtersData() {
  ...
  if (hasUntagged) {
    ...
    typeFilters += `<label class="cal-filter-item${off ? ' cal-filter-off' : ''}" data-type="__untagged__">
      <span class="cal-filter-dot" style="background:${_TYPE_PALETTE.untagged}"></span>senza tag</label>`;
  }
...
function _filtersToggleHTML() {
  ...
  return `<button class="cal-filter-toggle" id="cal-filter-toggle" title="${_filtersCollapsed ? 'Mostra filtri' : 'Nascondi filtri'}">${_filtersCollapsed ? '+ tags' : '− tags'}</button>`;
}
    ...
    for (let col = 0; col < 7; col++) {
      ...
      if (singles.length) {
        ...
        for (const ev of showInline) {
          ...
          const _impMark = ev.importance === 'critical' ? '<span style="color:var(--red);margin-right:2px" title="critico">!!</span>'
                         : ev.importance === 'high' ? '<span style="color:var(--orange,#e5a33a);margin-right:2px" title="alto">!</span>' : '';
...
async function _renderWeek() {
  ...
  let railHtml = `<div class="cal-wk-rail">
    <div class="cal-wk-rail-spacer">
      <button class="cal-wk-zoom" id="cal-wk-zoom-out" title="Zoom out (–)" aria-label="Riduci">−</button>
      <button class="cal-wk-zoom" id="cal-wk-zoom-in" title="Ingrandisci (+)" aria-label="Ingrandisci">+</button>
    </div>`;
  ...
  for (const { d, ds, idx } of days) {
    ...
    for (const ev of timedEvents) {
      ...
      colsHtml += `<div class="cal-wk-block-resize" title="Trascina per ridimensionare"></div>`;
      colsHtml += `</div>`;
    }
    ...
    block.addEventListener('mousedown', (e) => {
      ...
      const onUp = async (up) => {
        ...
        try {
          ...
          _render();
          _showCalUndoToast('Evento spostato', async () => {
            try {
              await _updateEvent(uid, { dtstart: prevDtstart, dtend: prevDtend });
              _render();
            } catch (err) { console.error('Annullamento non riuscito:', err); }
          });
        } catch {
          _render();
        }
    ...
    handle.addEventListener('mousedown', (e) => {
      ...
      const onUp = async () => {
        ...
        try {
          ...
          _render();
          _showCalUndoToast('Evento ridimensionato', async () => {
            try {
  ...
  if (!dates.length) {
    ...
    h += '<div class="cal-empty" style="display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;">' +
      ...
      '<span style="opacity:0.7;font-size:11px;">' +
        '<a href="#" data-cal-open-settings="integrations" style="color:var(--accent,var(--red));text-decoration:underline;">Impostazioni &rsaquo; Integrazioni</a>' +
        ' &middot; ' +
        '<a href="#" data-cal-create-event="1" style="color:var(--accent,var(--red));text-decoration:underline;">Crea evento</a>' +
      '</span>' +
    '</div>';
  } else {
    for (const date of dates) {
      const evs = byDate.get(date);
      const todayBadge = (date === today) ? ' <span class="cal-agenda-today-badge">Oggi</span>' : '';
      h += `<div class="cal-agenda-day${date === today ? ' is-today' : ''}"><div class="cal-agenda-date">${_fmtDate(date)}${todayBadge}</div>`;
      if (!evs.length) {
        h += '<div class="cal-agenda-empty">Nessun evento</div>';
      }
      for (const ev of evs) {
        const t = ev.all_day ? 'Tutto il giorno' : _fmtTime(ev.dtstart) + ' – ' + _fmtTime(ev.dtend);
        const _typeTag = ev.event_type
          ...
          : '';
        const _impMark = ev.importance === 'critical' ? '<span style="color:var(--red);margin-right:4px" title="critico">!!</span>'
                       : ev.importance === 'high' ? '<span style="color:var(--orange,#e5a33a);margin-right:4px" title="alto">!</span>' : '';
        h += `<div class="cal-agenda-event" data-uid="${_e(ev.uid)}">
          ...
          <button class="cal-event-more" data-uid="${_e(ev.uid)}" title="Altro">${_moreIcon}</button>
        </div>`;
      }
      h += '</div>';
    }
...
async function _renderSearch() {
  ...
  if (!results.length) {
    h += '<div class="cal-empty">Nessun evento corrisponde alla ricerca</div>';
  } else {
...
function _dayDetailHTML(dateStr) {
  ...
  const searchInput = `<div class="cal-search-wrap">
    ...
    <input type="search" class="cal-search-input cal-day-search" id="cal-search" placeholder="Cerca tutti gli eventi…" value="${_e(_searchQuery)}" />
  </div>`;
  let h = `<div class="cal-splitter" role="separator" aria-orientation="horizontal" tabindex="0" title="Trascina per ridimensionare"><div class="cal-splitter-grip"></div></div>
    <div class="cal-day-detail">
    ${searchInput}
    <div class="cal-detail-header">
      <span>${_fmtDate(dateStr)}${isToday ? ' <span style="color:var(--accent, var(--red));font-weight:600;">(Oggi)</span>' : ''}</span>
      <button class="cal-add-btn cal-add-btn-text cal-add-btn-sm" id="cal-add-day" title="Nuovo evento"><span class="cal-add-plus">+</span><span class="cal-add-label">Nuovo</span></button>
    </div>`;
  if (_searchQuery) {
    const q = _searchQuery.toLowerCase();
    ...
    if (!results.length) {
      h += '<div class="cal-empty">Nessun evento corrisponde</div>';
    } else {
      results.forEach(ev => {
...
function _wireAll(body) {
  ...
  try {
    ...
    if (calBody && splitter) {
      ...
      if (!alreadySet) {
        const saved = parseInt(localStorage.getItem('argodesk.cal.detailH') || '0', 10);
      ...
      const onUp = () => {
        ...
        if (px) { try { localStorage.setItem('argodesk.cal.detailH', String(px)); } catch {} }
      };
      const onDown = (ev) => {
        ev.preventDefault();
        dragging = true;
      ...
      const resetSplit = () => {
        ...
        try { localStorage.removeItem('argodesk.cal.detailH'); } catch {}
      };
    ...
    const _submitQA = async () => {
      ...
      try {
        ...
        if (!res.ok || !data.ok) {
          ...
          uiModule.showError('Aggiunta rapida: ' + (data.error || data.detail || `HTTP ${res.status}`));
          return;
        }
        ...
        uiModule.showError('Aggiunta rapida non riuscita: ' + e.message);
      } finally {
        _qaSubmitting = false;
  ...
  document.getElementById('cal-sync')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (uiModule?.showToast) uiModule.showToast('Calendario aggiornato');
    }
  });
...
async function _showCalSettings() {
  ...
  overlay.innerHTML = `
    <div class="modal-content" style="width:420px;max-width:92vw;">
      ...
      <div class="modal-body" style="padding:16px;display:flex;flex-direction:column;gap:16px;">
        <div>
          <div style="font-size:11px;opacity:0.5;margin-bottom:6px;">I tuoi calendari</div>
        ...
        <div style="border-top:1px solid var(--border);padding-top:12px;">
          <div style="font-size:11px;opacity:0.5;margin-bottom:6px;">Importa calendario</div>
          <div style="display:flex;gap:8px;align-items:center;">
            <label class="memory-toolbar-btn" style="cursor:pointer;">
              ...
              <span style="position:relative;top:4px;">Importa .ics</span>
        ...
        <div style="border-top:1px solid var(--border);padding-top:12px;">
          <div style="font-size:11px;opacity:0.5;margin-bottom:6px;">Esporta calendario</div>
        ...
        <div style="border-top:1px solid var(--border);padding-top:12px;">
          <div style="font-size:11px;opacity:0.5;margin-bottom:6px;">Sincronizza</div>
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
            <button class="memory-toolbar-btn" id="cal-settings-sync-now" style="cursor:pointer;">
              ...
              <span style="position:relative;top:1px;">Sincronizza ora</span>
            </button>
  ...
  overlay.querySelector('#cal-settings-add')?.addEventListener('click', async (e) => {
    ...
    try {
      const r = await fetch(`${API_BASE}/api/calendar/calendars?name=${encodeURIComponent('Nuovo calendario')}&color=${encodeURIComponent(color)}`, { method: 'POST', credentials: 'same-origin' });
      const d = await r.json().catch(() => ({}));
      if (!r.ok || !d.ok) throw new Error(d.error || 'Creazione del calendario non riuscita');
      ...
      btn.disabled = false;
      if (window.showError) window.showError(err.message || 'Creazione del calendario non riuscita');
  ...
  overlay.querySelectorAll('.cal-settings-row').forEach(row => {
    ...
    delBtn.addEventListener('click', async () => {
      ...
      if (!await window.styledConfirm(`Eliminare il calendario "${name}" e tutti i suoi eventi?`, { confirmText: 'Elimina', danger: true })) return;
  ...
  overlay.querySelector('#cal-import-file').addEventListener('change', async (e) => {
    ...
    try {
      ...
      // generic "Importazione non riuscita".
      let data = null, raw = '';
      ...
      if (res.ok && data && data.ok) {
        ...
        console.error('Importazione del calendario non riuscita', res.status, data || raw);
      }
    } catch (err) {
      status.textContent = `Import failed: ${err.message || err}`;
      console.error('Errore durante l’importazione del calendario', err);
    }
    e.target.value = '';
  });
  ...
  overlay.querySelector('#cal-settings-sync-now')?.addEventListener('click', async (e) => {
    ...
    if (data.errors && data.errors.length) {
      ...
      status.textContent = parts.length ? `Synced — ${parts.join(', ')}` : 'Sincronizzato — nessuna modifica';
      _allEvents = {}; _fetchedRanges = [];
      try { localStorage.removeItem(LS_KEY); } catch (_) {}
      await _fetchCalendars();
      _render();
...
function _showEventForm(existing, defaultDate, defaultEndDate) {
  ...
  // title or clicking "Aggiungi dettagli" reveals it. Empty drafts feel like a
  ...
  body.innerHTML = `<div class="cal-form cal-form-bespoke${_expandedAtStart ? ' is-expanded' : ''}">
    <button type="button" class="cal-form-mobile-cancel" id="cal-form-mobile-cancel" title="Annulla" aria-label="Annulla evento">
    ...
    <div class="cal-form-today" id="cal-form-today">Oggi è <span id="cal-form-today-text">${_clockDate(_today())} · ${_nowClock()}</span></div>
    <div class="cal-hero">
      <button type="button" class="cal-hero-time" id="cal-hero-time" title="Cambia orario">
      ...
      <button type="button" class="cal-hero-date" id="cal-hero-date" title="Cambia data">${_clockDate(ds)}</button>
    </div>

    <div class="cal-title-wrap">
      <input type="text" id="cal-f-sum" placeholder=" " value="${_e(existing?.summary || '')}" class="cal-input cal-hero-title" autocomplete="off" />
      <span class="cal-title-hint" aria-hidden="true">${isEdit ? 'Titolo evento' : 'Cosa succede?'}<svg class="cal-title-enter-ico" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 10 4 15 9 20"/><path d="M20 4v7a4 4 0 0 1-4 4H4"/></svg></span>
    </div>
    ...
    <div class="cal-form-details" id="cal-form-details" aria-hidden="${_expandedAtStart ? 'false' : 'true'}">
      <div class="cal-form-row">
        ...
        <div class="cal-allday-ctrl">
          <span class="cal-allday-label">Tutto il giorno</span>
      ...
      <div class="cal-loc-row">
        <input type="text" id="cal-f-loc" placeholder="Luogo" value="${_e(existing?.location || '')}" class="cal-input" />
        <a id="cal-f-loc-map" class="cal-loc-map" href="#" target="_blank" rel="noopener noreferrer" title="Apri in Maps" aria-label="Apri in Apple Maps" tabindex="-1">
      ...
      <select id="cal-f-rrule" class="cal-input">
        <option value="" ${!existing?.rrule ? 'selected' : ''}>Non si ripete</option>
        <option value="FREQ=DAILY" ${existing?.rrule === 'FREQ=DAILY' ? 'selected' : ''}>Giornaliero</option>
        <option value="FREQ=WEEKLY" ${existing?.rrule === 'FREQ=WEEKLY' ? 'selected' : ''}>Settimanale</option>
        <option value="FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR" ${existing?.rrule === 'FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR' ? 'selected' : ''}>Giorni feriali</option>
        <option value="FREQ=MONTHLY" ${existing?.rrule === 'FREQ=MONTHLY' ? 'selected' : ''}>Mensile</option>
        <option value="FREQ=YEARLY" ${existing?.rrule === 'FREQ=YEARLY' ? 'selected' : ''}>Annuale</option>
      </select>
      <textarea id="cal-f-desc" placeholder="Descrizione" class="cal-input" rows="2">${_e(existing?.description || '')}</textarea>
      <div class="cal-form-row" style="align-items:center;gap:8px;">
        <label style="font-size:11px;display:flex;align-items:center;gap:4px;"><svg class="cal-remind-bell" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--accent, var(--red))" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg><span style="opacity:0.5;">Promemoria</span></label>
        <select id="cal-f-remind" class="cal-input" style="flex:1;">
          <option value="" ${isEdit ? 'selected' : ''}>Nessun promemoria</option>
          ...
          <option value="custom">Orario esatto...</option>
        </select>
        <input type="datetime-local" id="cal-f-remind-custom" class="cal-input" style="flex:1;display:none;" />
      </div>
      <div class="cal-form-row" style="align-items:center;gap:8px;">
        <label style="font-size:11px;opacity:0.5;">Colore</label>
  ...
  document.getElementById('cal-f-save')?.addEventListener('click', async () => {
    ...
    if (!summary) { uiModule.showToast('Il titolo è obbligatorio'); return; }
    ...
    } catch (e) { uiModule.showToast('Salvataggio non riuscito'); }
  });
  document.getElementById('cal-f-del')?.addEventListener('click', async () => {
    ...
    catch (e) { uiModule.showToast('Eliminazione non riuscita'); }
  });
  ...
  const _syncHero = () => {
    ...
    if (clockEl) clockEl.innerHTML = allday ? '<span class="cal-hero-clock-allday">Tutto il giorno</span>' : _clockFace(startVal);
...
function _fmtDate(s) { return new Date(s + 'T00:00:00').toLocaleDateString('it-IT', { weekday: 'long', month: 'long', day: 'numeric' }); }
...
function _clockDate(ds) {
  if (!ds) return '';
  return new Date(ds + 'T00:00:00').toLocaleDateString('it-IT', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' });
}
function _nowClock() {
  // Live wall-clock string for the "Today is …" header. Locale-aware so
  // 24-h users don't see AM/PM.
  return new Date().toLocaleTimeString('it-IT', { hour: 'numeric', minute: '2-digit' });
}
function _fmtTime(s) {
...
function _locHTML(loc) {
  ...
  return `<a href="${mapUrl}" target="_blank" rel="noopener" onclick="event.stopPropagation();" title="Apri in OpenStreetMap">${_e(loc)}</a>`;
}
...
const LS_KEY = 'argodesk-calendar-cache';
