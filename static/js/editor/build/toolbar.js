 ...
 */
import { ADVANCED_EDITING } from '../editor-config.js';
...
export function buildToolbar({ currentTool, onSelectTool, onClearSelection }) {
  ...
  let tools = [
    ...
    { sep: true, adv: true },
    { id: 'clone', label: 'Clone', adv: true, icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="9" r="3"/><path d="M9 12l-3 4h12l-3-4"/><path d="M4 20h16"/></svg>', key: 'K' },
    { id: 'lasso', label: 'Lasso', adv: true, icon: '⟡', key: 'L' },
    { id: 'wand', label: 'Wand', adv: true, icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 4V2"/><path d="M15 16v-2"/><path d="M8 9h2"/><path d="M20 9h2"/><path d="M17.8 11.8L19 13"/><path d="M15 9h0"/><path d="M17.8 6.2L19 5"/><path d="M3 21l9-9"/><path d="M12.2 6.2L11 5"/></svg>', key: 'W' },
    { sep: true, adv: true },
    { id: 'inpaint', label: 'Inpaint', ai: true, adv: true, icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9.06 11.9l8.07-8.06a2.85 2.85 0 1 1 4.03 4.03l-8.06 8.08"/><path d="M7.07 14.94c-1.66 0-3 1.35-3 3.02 0 1.33-2.5 1.52-2 2.02 1.08 1.1 2.49 2.02 4 2.02 2.2 0 4-1.8 4-4.04a3.01 3.01 0 0 0-3-3.02z"/></svg>', key: 'M' },
    { id: 'rembg', ai: true, adv: true, label: 'Rimuovi sfondo', icon: '✄' },
    { id: 'sharpen', ai: true, adv: true, label: 'Sharpen', icon: '◈', key: 'S' },
  ];
  // Minimal-editor mode: drop the advanced tools, then collapse any
  // separator left dangling (leading / trailing / doubled) by the cut.
  if (!ADVANCED_EDITING) {
    tools = tools.filter(t => !t.adv);
    tools = tools.filter((t, i) => !(t.sep && (i === 0 || i === tools.length - 1 || tools[i - 1]?.sep)));
  }
  const toolKeyMap = {};
