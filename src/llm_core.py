...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt ArgoDesk' canonical OpenAI-style messages to native Ollama /api/chat.

    ArgoDesk carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://argodesk.local")
        h.setdefault("X-OpenRouter-Title", "ArgoDesk")
    return h
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip ArgoDesk-only metadata before sending messages to providers.
